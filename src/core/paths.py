...
APP_VERSION = "1.2.0"  # keep in sync with pyproject [project].version
...
def resource_root() -> Path:
    """Base dir holding bundled resources (calibration.toml, data/...)."""
    if _frozen():
        # _MEIPASS in onefile / the exe dir in onedir; both hold the --add-data tree.
        return Path(getattr(sys, "_MEIPASS", Path(sys.executable).resolve().parent))
    return Path(__file__).resolve().parent.parent.parent  # repo root (src/core/ -> ../../..)
