...
def test_no_reading_is_noop():
    t = _tracker(3)
    out = apply_chat_turn(t, None, asleep=False, now=10.0, last_advance=0.0, down_guard_s=3.0, battle_start=0.0, start_grace_s=3.0)
    assert out == "none"
...
def test_ignored_during_turn_one():
    ...
    t = _tracker(0)
    out = apply_chat_turn(t, 11, asleep=False, now=10.0, last_advance=0.0, down_guard_s=3.0, battle_start=8.0, start_grace_s=3.0)
    assert out == "start"
...
def test_up_correction_for_missed_turn():
    ...
    out = apply_chat_turn(t, 4, asleep=False, now=10.0, last_advance=0.0, down_guard_s=3.0, battle_start=0.0, start_grace_s=3.0)
    assert out == "up"
    assert t.turns_completed == 3
...
def test_down_correction_blocked_within_guard():
    ...
    t = _tracker(5)  # counter at turn 6
    out = apply_chat_turn(t, 4, asleep=False, now=10.5, last_advance=10.0, down_guard_s=3.0, battle_start=0.0, start_grace_s=3.0)
...
def test_sleep_turns_accrue_while_asleep():
    ...
    t = _tracker(2)  # counter at turn 3
    apply_chat_turn(t, 3, asleep=True, now=10.0, last_advance=0.0, down_guard_s=3.0, battle_start=0.0, start_grace_s=3.0)  # sync
    before = t.turns_asleep
    apply_chat_turn(t, 4, asleep=True, now=10.0, last_advance=0.0, down_guard_s=3.0, battle_start=0.0, start_grace_s=3.0)  # +1 turn
    assert t.turns_asleep == before + 1


def test_sleep_resets_when_awake():
    t = _tracker(2)
    apply_chat_turn(t, 4, asleep=True, now=10.0, last_advance=0.0, down_guard_s=3.0, battle_start=0.0, start_grace_s=3.0)
    assert t.turns_asleep > 0
    apply_chat_turn(t, 5, asleep=False, now=10.0, last_advance=0.0, down_guard_s=3.0, battle_start=0.0, start_grace_s=3.0)
